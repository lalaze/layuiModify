...
layui.define(['layer', 'laytpl'], function (exports) {
  ...
  Class.prototype.upload = function (files, type) {
    ...
    var that = this,
      ...
      ,
      // 默认回调
      // id再请求  太几把难了，对比前面，其实就是url变了而已，再来个ajax放进去试试
      urlCallback = function (index, res) {
        that.elemFile.next('.' + ELEM_CHOOSE).remove();
        elemFile.value = '';
        if (typeof res !== 'object') {
          try {
    ...
    switch (options.accept) {
      ...
      case 'audio': //音频文件
        ...
        break;
      case 'pdf':
        if (!RegExp('\\w\\.(' + (exts || 'pdf') + ')$', 'i').test(escape(value))) {
          that.msg('选择的文件不为pdf');
          return elemFile.value = '';
        }
        break;
    ...
    switch (options.accept) {
      case 'file': //一般文件
        if (!(exts && !RegExp('\\w\\.(' + exts + ')$', 'i').test(escape(value)))) {
          options.data.scourceType = 0;
        }
        break;
      case 'video': //视频文件
        if (!(!RegExp('\\w\\.(' + (exts || 'avi|mp4|wma|rmvb|rm|flash|3gp|flv') + ')$', 'i').test(escape(value)))) {
          options.data.scourceType = 2;
        }
        break;
      case 'audio': //音频文件
        if (!(!RegExp('\\w\\.(' + (exts || 'mp3|wav|mid') + ')$', 'i').test(escape(value)))) {
